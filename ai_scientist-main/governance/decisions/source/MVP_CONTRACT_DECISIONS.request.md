...
请为 D-001、D-002、D-003 各选择一个方案，并确认 `actor_id`。可以直接回复：
...
确认后，Codex 会把选择写入决策记录，更新契约草案，再给你做第二轮逐节审阅。没有你的明确确认，不会将本文件改为 `ACCEPTED`。
...
- `FIXTURE_IMPORT_REVIEW`：确认导入清单、哈希、缺失项和兼容性问题；
- `FINAL_REPLAY_REVIEW`：确认最终回放报告可以进入 `READY_FOR_INTERNAL_DEMO`，仅限内部开发性演示。
